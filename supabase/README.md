...
supabase functions deploy move-routine
```

`supabase/config.toml` deja `verify_jwt = false` para `import-excel`, `archive-event`, `delete-routine`, `delete-judge`, `upsert-judge`, `update-routine` y `move-routine` porque esta app todavia no usa Supabase Auth. `import-excel` valida el header `x-import-secret`; para esta instalacion esa clave es `levitate2026`. `archive-event`, `delete-routine`, `delete-judge`, `upsert-judge`, `update-routine` y `move-routine` no piden clave desde la app. Despues usan una secret key disponible en el runtime de Supabase para escribir tablas admin. No pongas esa secret key en iOS.
