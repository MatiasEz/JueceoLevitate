...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const payload = await request.json() as DeleteRoutinePayload;
    ...
    const routineRows = await supabaseRequest(
      "GET",
      `routines?select=event_id,routine_id,name&event_id=eq.${encodeURIComponent(eventID)}&routine_id=eq.${encodeURIComponent(routineID)}&limit=1`,
    ) as AnyRow[];
...
function authorizeImport(request: Request): void {
  ...
  if (!safeEqual(provided, expected)) {
    throw new HTTPError(403, "No tenes permiso para borrar coreografias.");
  }
}
